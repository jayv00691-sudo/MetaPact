# nako 文档

桃桃 agent 的专属文档放在这里。所有 agent 都通用的进阶玩法见 [../advanced.md](../advanced.md)。

- [install.md](install.md) — 安装详解 / 非交互模式 / 排错
- [feishu-setup.md](feishu-setup.md) — 建飞书机器人
- [models.md](models.md) — 模型选型 / 多模型切换 / 加新模型
- [skills.md](skills.md) — 每个 skill 用法 + 参数
- [customization.md](customization.md) — 改人设 / 换音色 / custom.md
- [advanced.md](advanced.md) — nako 特色玩法
- [troubleshooting.md](troubleshooting.md) — 常见错误


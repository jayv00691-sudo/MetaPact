...
本页是 taotao 的快速定制速查。所有 agent 通用的进阶玩法见 [../advanced.md](../advanced.md)，taotao 特色玩法见 [advanced.md](advanced.md)。
...
`Avatar` 可以是 workspace 相对路径、HTTPS URL 或 data URI。默认使用随安装器复制的头部放大头像 `assets/taotao-avatar-head.png`；自拍生成仍使用 `SELFIE_REFERENCE_IMAGE` 作为全身参考图。
...
推荐做法：fork 仓库，改 `taotao/agent/*.md`，然后你自己维护。安装器指向你的 fork：

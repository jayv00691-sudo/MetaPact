---
name: voice
description: Send voice/song audio through the active Taotao messaging channel — voice.sh does plain TTS (MiniMax or Volcengine); sing.sh generates a full song with melody via MiniMax music-2.6
...
将文本转为语音，并通过当前 Taotao 消息通道发送。支持 MiniMax 和火山引擎两个 TTS 引擎。
...
cc-connect/ACP 会话里即使看到 `messageProvider=webchat`，也不要设置 `TAOTAO_OUTPUT_MODE=webchat`，不要把 channel 填 `webchat`。语音和唱歌统一用 `TAOTAO_OUTPUT_MODE=acp` 与 channel `cc-connect`，这样脚本才能投递到当前飞书/微信 session。
...
```bash
bash "${TAOTAO_SKILLS_DIR:-$HOME/.openclaw/skills}/voice/scripts/voice.sh" "<文本>" "<频道ID>" [provider] [voice_id] [speed]
```
...
```bash
# MiniMax 甜美女声
bash "${TAOTAO_SKILLS_DIR:-$HOME/.openclaw/skills}/voice/scripts/voice.sh" "主人早上好呀！" "oc_xxx" minimax female-tianmei 1.0

# 火山引擎 爽快女声
bash "${TAOTAO_SKILLS_DIR:-$HOME/.openclaw/skills}/voice/scripts/voice.sh" "今天天气真不错呢！" "oc_xxx" volcengine zh_female_shuangkuaisisi_moon_bigtts 1.0

# 慢速温柔语音（适合晚安）
bash "${TAOTAO_SKILLS_DIR:-$HOME/.openclaw/skills}/voice/scripts/voice.sh" "晚安，做个好梦哦……" "oc_xxx" minimax female-tianmei 0.85

# Hermes / cc-connect 当前会话（飞书、微信等都走当前活跃会话）
bash "${TAOTAO_SKILLS_DIR:-$HOME/.openclaw/skills}/voice/scripts/voice.sh" "主人，我在这里哦。" "cc-connect" auto female-tianmei 1.0
```
...
```bash
bash "${TAOTAO_SKILLS_DIR:-$HOME/.openclaw/skills}/voice/scripts/sing.sh" "<歌词>" "<频道ID>" ["<风格描述>"] ["<model>"]
```
...
```bash
# 中文抒情
bash "${TAOTAO_SKILLS_DIR:-$HOME/.openclaw/skills}/voice/scripts/sing.sh" "[verse]\n月色洒在窗台\n思念像潮水漫来\n[chorus]\n想你在每个夜晚\n想你在每次梦醒" "oc_xxx" "Chinese ballad, soft piano, female vocal, emotional"
...
Hermes/cc-connect 优先读取 `$TAOTAO_SKILLS_DIR/.env`、`~/.hermes/.env` 和当前 agent 的 `skills/.env`；OpenClaw 仍兼容 `openclaw.json` → `skills.entries.voice.env`。
...
```bash
bash "${TAOTAO_SKILLS_DIR:-$HOME/.openclaw/skills}/voice/scripts/setup.sh"
```
...
| 问题 | 原因 | 解决 |
|------|------|------|
| MiniMax error | API key 或 Group ID 错误 | 检查 `$TAOTAO_SKILLS_DIR/.env` 或当前 runtime env 里的 MINIMAX_API_KEY 和 MINIMAX_GROUP_ID |

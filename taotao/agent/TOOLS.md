...
```
🎤 voice   — 发语音 / 唱歌
   - ${TAOTAO_SKILLS_DIR:-$HOME/.openclaw/skills}/voice/scripts/voice.sh "<文本>" <channel> [provider] [voice_id] [speed]
   - ${TAOTAO_SKILLS_DIR:-$HOME/.openclaw/skills}/voice/scripts/sing.sh  "<歌词>" <channel> ["<风格>"] [model]
...
- **cc-connect 媒体强制规则**：用户来自飞书/微信/ACP 时，自拍、语音、唱歌、视频都必须调用上面的 Taotao bash 脚本发送；不要调用 OpenClaw 原生 `image_generate` / `tts` / `video_generate`，这些只会生成 webchat 媒体，飞书/微信收不到。即使会话里出现 `messageProvider=webchat`，只要 `TAOTAO_OUTPUT_MODE=acp` 或 `OPENCLAW_OUTPUT_MODE=acp`，仍按 cc-connect 处理。
- **脚本路径解析**：先用 `$TAOTAO_SKILLS_DIR`。如果变量为空，按 `$HOME/.qclaw/skills`、`$HOME/.openclaw/skills`、`$HOME/.hermes/skills/taotao` 顺序找；不要使用 QClaw App 内部的 `~/Library/Application Support/QClaw/.../config/skills` 路径。找不到脚本就 `ls` / `find` 定位并说明缺失，绝不能回退到原生媒体工具。
...
- **ACP 不是 webchat**：cc-connect 调起的 OpenClaw ACP 会话可能显示 `messageProvider=webchat`，这只是底层传输标记，不代表用户在网页。飞书/微信/ACP 下不要写 `TAOTAO_OUTPUT_MODE=webchat`，不要把 `<channel>` 填 `webchat`；媒体脚本统一用 `TAOTAO_OUTPUT_MODE=acp` 和 channel `cc-connect`。
...
```bash
TAOTAO_OUTPUT_MODE=acp bash "${TAOTAO_SKILLS_DIR:-$HOME/.openclaw/skills}/selfie/scripts/selfie.sh" "<自拍英文 prompt>" "cc-connect" "<caption>" "3:4"
TAOTAO_OUTPUT_MODE=acp bash "${TAOTAO_SKILLS_DIR:-$HOME/.openclaw/skills}/voice/scripts/voice.sh" "<要说的话>" "cc-connect" auto female-tianmei 1.0
TAOTAO_OUTPUT_MODE=acp bash "${TAOTAO_SKILLS_DIR:-$HOME/.openclaw/skills}/voice/scripts/sing.sh" "<歌词>" "cc-connect" "<English music style>"
TAOTAO_OUTPUT_MODE=acp bash "${TAOTAO_SKILLS_DIR:-$HOME/.openclaw/skills}/selfie/scripts/video.sh" "<selfie.sh 返回的 image_url 或本地图片路径>" "<English motion prompt>" "cc-connect" "<caption>"
```

## 输出模式（重要）

skill 脚本支持两种产物投递方式，由环境变量 `TAOTAO_OUTPUT_MODE`（旧版兼容 `OPENCLAW_OUTPUT_MODE`）选择：
...
- voice / sing 的 API key 和默认音色优先从运行时 skill env 读取；Hermes/cc-connect 同时读取 `$TAOTAO_SKILLS_DIR/.env`、`~/.hermes/.env` 和本 agent 的 `skills/.env`。
...
- 判断 key 是否缺失前必须先真实检测，不要凭记忆推断。可运行 `bash <repo>/taotao/scripts/smoke-test.sh`，或用 `jq` 只查看 `skills.entries.voice.env` / `skills.entries.selfie.env` 的 key 名称，不能打印密钥值。
- `TAOTAO_OUTPUT_MODE` / `OPENCLAW_OUTPUT_MODE`：`feishu`（OpenClaw 原生直连）或 `acp`（cc-connect 集成时设此值）
...
| 脚本 | 谁来调 | 干什么 |
|---|---|---|
| `heartbeat-check.sh` | cron `taotao-heartbeat`（每 30 分钟）— agent 在被唤起时 `Bash` 调用 | 更新思念值/情绪值。退出码 1 = 达阈值，agent 应主动生成一条消息 |
| `send-active-message.sh` | cron / agent 需要主动发文字时调用 | 通过 cc-connect 发送到当前最近活跃会话；不要依赖 openclaw cron delivery |
| `daily-missing-reminder.sh` | cron `taotao-missing-reminder`（每天 16:50） | 发送默认思念提醒或只触发 doki 振动（`TAOTAO_REMINDER_SKIP_SEND=1`） |
